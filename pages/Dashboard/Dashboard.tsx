...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeMsgs = onSnapshot(qMessages,
      ...
      (err) => {
        ...
        if (err.code === 'permission-denied') setError('No tienes permisos para ver los mensajes.');
      }
    );
  ...
  const handleReplyMessage = (message: ContactMessage) => {
    ...
    if (message.status === 'new') handleToggleMessageRead(message.id, 'new');
  };
  ...
  const confirmStatusAndApply = () => {
    if (confirmStatusChange) handleUpdateStatus(confirmStatusChange.id, confirmStatusChange.status, true);
    setConfirmStatusChange(null);
  };
      ...
      <nav aria-hidden={!mobileMenuOpen} className={`lg:hidden fixed top-0 left-0 h-full w-[260px] max-w-[85vw] bg-white shadow-2xl z-50 flex flex-col pt-[60px] transition-transform duration-300 ease-out overflow-hidden ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col p-4 gap-1 overflow-y-auto">
          <button onClick={() => { setActiveTab('appointments'); setMobileMenuOpen(false); }} className={`flex w-full items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-left ${activeTab === 'appointments' ? 'text-primary bg-blue-50' : 'text-slate-600 hover:bg-slate-50'}`}>
            <span className="material-symbols-outlined">event_note</span> Citas
          </button>
          <button onClick={() => { setActiveTab('patients'); setMobileMenuOpen(false); }} className={`flex w-full items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-left ${activeTab === 'patients' ? 'text-primary bg-blue-50' : 'text-slate-600 hover:bg-slate-50'}`}>
            <span className="material-symbols-outlined">person_outline</span> Pacientes
            {patients.length > 0 && <span className="ml-auto bg-slate-200 text-slate-600 text-[10px] px-2 py-0.5 rounded-full">{patients.length}</span>}
          </button>
          <button onClick={() => { setActiveTab('messages'); setMobileMenuOpen(false); }} className={`flex w-full items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-left ${activeTab === 'messages' ? 'text-primary bg-blue-50' : 'text-slate-600 hover:bg-slate-50'}`}>
            <span className="material-symbols-outlined">mail_outline</span> Mensajes Web
        ...
        <div className="p-4 sm:p-6 lg:p-10 bg-[#f8fafc] overflow-y-auto flex-1 overflow-x-hidden">
          <div className="max-w-[1400px] mx-auto space-y-6 sm:space-y-8">
            <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4 lg:gap-4">
              <StatsCard label="CITAS HOY" value={stats.totalToday} icon="calendar_today" color="bg-primary" />
              <StatsCard label="PENDIENTES" value={stats.pending} icon="assignment_late" color="bg-amber-500" />
              <StatsCard label="MENSAJES" value={stats.newMessages} icon="mail" color="bg-blue-500" />
              <StatsCard label="REALIZADAS" value={stats.completed} icon="check_circle" color="bg-emerald-500" />
            </div>
            ...
            {activeTab === 'appointments' && (
              <AppointmentCalendar
                ...
                groupedAppointmentsForList={groupedAppointmentsForList}
                ...
                onHoverAppointment={setHoveredAppId}
            ...
            {activeTab === 'patients' && (
              <PatientList
                ...
                importingFromCalendar={importingFromCalendar}
                onNewPatient={openPatientCreate}
                onEditPatient={openPatientEdit}
                onDeletePatient={handleDeletePatient}
                onImportFromCalendar={handleImportPatientsFromCalendar}
                  ...
                  {messages.map((msg) => (
                    <div key={msg.id} className={`group bg-white p-8 rounded-3xl border transition-all shadow-sm flex flex-col md:flex-row justify-between gap-6 relative overflow-hidden ${msg.status === 'new' ? 'border-primary/20 bg-gradient-to-br from-white to-indigo-50/20' : 'border-slate-100'}`}>
                      ...
                      <div className="flex md:flex-col items-center justify-end gap-3 shrink-0">
                        <button onClick={() => handleReplyMessage(msg)} className="flex-1 md:w-full px-5 py-3 rounded-2xl bg-slate-900 text-white font-bold text-xs flex items-center justify-center gap-2 hover:bg-black transition-all shadow-lg shadow-slate-200">
                          <span className="material-symbols-outlined text-sm">reply</span> Responder
                        </button>
                        <div className="flex gap-2 w-full md:w-auto">
                          <button onClick={() => handleToggleMessageRead(msg.id, msg.status)} title={msg.status === 'new' ? 'Marcar como leído' : 'Marcar como no leído'} className={`flex-1 size-11 rounded-2xl flex items-center justify-center border-2 transition-all ${msg.status === 'new' ? 'border-slate-100 bg-white text-slate-400 hover:text-primary hover:border-primary/20' : 'border-primary/10 bg-indigo-50 text-primary'}`}>
                            <span className="material-symbols-outlined text-xl">{msg.status === 'new' ? 'check' : 'visibility_off'}</span>
                          </button>
                          <button onClick={() => handleDeleteMessage(msg.id)} title="Eliminar mensaje" className="flex-1 size-11 rounded-2xl border-2 border-slate-100 bg-white text-rose-400 flex items-center justify-center hover:bg-rose-50 hover:border-rose-100 transition-all">
            ...
            <aside className="fixed top-0 right-0 h-full w-full lg:max-w-[500px] lg:w-[500px] bg-white shadow-2xl z-50 flex flex-col animate-slide-in-right overflow-y-auto pt-[env(safe-area-inset-top)]">
              ...
              <div className="p-4 sm:p-6 lg:p-8 flex-1 space-y-6 sm:space-y-8 overflow-y-auto pb-[env(safe-area-inset-bottom)]">
                <div className={`p-6 sm:p-8 rounded-2xl sm:rounded-[2.5rem] text-white shadow-xl transition-all duration-500 ${activeApp.status === AppointmentStatus.REALIZED ? 'bg-emerald-600' : activeApp.status === AppointmentStatus.NO_SHOW ? 'bg-rose-600' : 'bg-slate-900'}`}>
                  ...
                  <button onClick={() => handleWhatsAppReminder(activeApp)} className="mt-6 w-full py-4 bg-[#25D366] hover:bg-[#128C7E] text-white rounded-2xl font-black text-sm flex items-center justify-center gap-3 shadow-lg shadow-emerald-500/20 transition-all active:scale-95">
                  ...
                  <button onClick={() => handleAddAppointmentToPatients(activeApp)} disabled={addingToPatients} className="mt-3 w-full py-3 bg-primary/20 hover:bg-primary/30 text-primary rounded-2xl font-bold text-sm flex items-center justify-center gap-2 transition-all disabled:opacity-50">
                    {addingToPatients ? <><span className="material-symbols-outlined animate-spin text-lg">progress_activity</span> Añadiendo...</> : <><span className="material-symbols-outlined text-lg">person_add</span> Añadir a Pacientes</>}
                  </button>
                  {addToPatientsFeedback === 'ok' && <p className="mt-2 text-sm font-bold text-emerald-600 flex items-center gap-2"><span className="material-symbols-outlined">check_circle</span> Añadido al registro</p>}
                  {addToPatientsFeedback === 'exists' && <p className="mt-2 text-sm font-bold text-amber-600 flex items-center gap-2"><span className="material-symbols-outlined">info</span> Ya está en el registro</p>}
                </div>
                <div className="space-y-6">
                  <div className="flex justify-between items-center px-2">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cambiar Estado de Cita</p>
                    {updatingStatus && <span className="flex items-center gap-2 text-primary text-[10px] font-black animate-pulse"><span className="material-symbols-outlined text-xs animate-spin">progress_activity</span> SINCRONIZANDO</span>}
                  </div>
                  {statusError && (
                    <div className="mx-2 p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center gap-3 text-rose-600 text-xs font-bold">
                      <span className="material-symbols-outlined text-lg">warning</span>
                      <p>{statusError}</p>
                    </div>
                  )}
                  <div className="grid grid-cols-1 gap-4">
                    <button disabled={updatingStatus} onClick={() => handleUpdateStatus(activeApp.id!, AppointmentStatus.PENDING)} className={`group w-full p-6 rounded-3xl border-2 transition-all text-left font-black flex items-center gap-5 ${activeApp.status === AppointmentStatus.PENDING ? 'border-slate-800 bg-slate-900 text-white shadow-lg' : 'border-slate-100 hover:border-slate-300 hover:bg-slate-50 text-slate-700'}`}>
                      <div className={`size-12 rounded-2xl flex items-center justify-center ${activeApp.status === AppointmentStatus.PENDING ? 'bg-white/10' : 'bg-slate-100 group-hover:bg-slate-200'}`}><span className="material-symbols-outlined">hourglass_empty</span></div>
                      <div className="flex-1"><p className="text-sm">Pendiente</p><p className={`text-[10px] font-bold ${activeApp.status === AppointmentStatus.PENDING ? 'text-white/60' : 'text-slate-400'}`}>Cita agendada, esperando ejecución.</p></div>
                      {activeApp.status === AppointmentStatus.PENDING && <span className="material-symbols-outlined text-white">check_circle</span>}
                    </button>
                    <button disabled={updatingStatus} onClick={() => handleUpdateStatus(activeApp.id!, AppointmentStatus.REALIZED)} className={`group w-full p-6 rounded-3xl border-2 transition-all text-left font-black flex items-center gap-5 ${activeApp.status === AppointmentStatus.REALIZED ? 'border-emerald-600 bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'border-slate-100 hover:border-emerald-200 hover:bg-emerald-50 text-slate-700'}`}>
                      <div className={`size-12 rounded-2xl flex items-center justify-center ${activeApp.status === AppointmentStatus.REALIZED ? 'bg-white/10' : 'bg-emerald-50 group-hover:bg-emerald-100'}`}><span className="material-symbols-outlined">task_alt</span></div>
                      <div className="flex-1"><p className="text-sm">Realizada</p><p className={`text-[10px] font-bold ${activeApp.status === AppointmentStatus.REALIZED ? 'text-white/60' : 'text-slate-400'}`}>Sesión terapéutica completada con éxito.</p></div>
                      {activeApp.status === AppointmentStatus.REALIZED && <span className="material-symbols-outlined text-white">check_circle</span>}
                    </button>
                    <button disabled={updatingStatus} onClick={() => handleUpdateStatus(activeApp.id!, AppointmentStatus.NO_SHOW)} className={`group w-full p-6 rounded-3xl border-2 transition-all text-left font-black flex items-center gap-5 ${activeApp.status === AppointmentStatus.NO_SHOW ? 'border-rose-600 bg-rose-600 text-white shadow-lg shadow-rose-100' : 'border-slate-100 hover:border-rose-200 hover:bg-rose-50 text-slate-700'}`}>
                      <div className={`size-12 rounded-2xl flex items-center justify-center ${activeApp.status === AppointmentStatus.NO_SHOW ? 'bg-white/10' : 'bg-rose-50 group-hover:bg-rose-100'}`}><span className="material-symbols-outlined">person_off</span></div>
                      <div className="flex-1"><p className="text-sm">No Asistió</p><p className={`text-[10px] font-bold ${activeApp.status === AppointmentStatus.NO_SHOW ? 'text-white/60' : 'text-slate-400'}`}>El paciente no se presentó al encuentro.</p></div>
                      {activeApp.status === AppointmentStatus.NO_SHOW && <span className="material-symbols-outlined text-white">check_circle</span>}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {confirmStatusChange && (
          <>
            ...
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl p-5 sm:p-8 z-[61] max-w-sm w-full mx-4">
              ...
              <p className="text-sm text-slate-500 mb-6">La cita se marcará como <strong>{confirmStatusChange.status === AppointmentStatus.REALIZED ? 'Realizada' : 'No asistió'}</strong>.</p>
          ...
          <>
            ...
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl p-5 sm:p-8 z-[61] max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto my-4">
              ...
              <form onSubmit={handleFormSubmit} className="space-y-4">
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Servicio</label><select value={formData.service} onChange={(e) => setFormData({ ...formData, service: e.target.value })} className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm">{SERVICES.map((s) => <option key={s} value={s}>{s}</option>)}</select>{formErrors.service && <p className="text-xs text-rose-500 mt-1">{formErrors.service}</p>}</div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nombre</label><input type="text" value={formData.patientName} onChange={(e) => setFormData({ ...formData, patientName: e.target.value })} placeholder="Nombre del paciente" className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" />{formErrors.patientName && <p className="text-xs text-rose-500 mt-1">{formErrors.patientName}</p>}</div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Email</label><input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="email@ejemplo.com" className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" />{formErrors.email && <p className="text-xs text-rose-500 mt-1">{formErrors.email}</p>}</div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Teléfono</label><input type="tel" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} placeholder="+56 9 ..." className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" />{formErrors.phone && <p className="text-xs text-rose-500 mt-1">{formErrors.phone}</p>}</div>
                <div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Fecha</label><input type="date" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" />{formErrors.date && <p className="text-xs text-rose-500 mt-1">{formErrors.date}</p>}</div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Hora</label><select value={formData.time} onChange={(e) => setFormData({ ...formData, time: e.target.value })} className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm">{FIXED_TIMES.map((t) => <option key={t} value={t}>{t}</option>)}</select></div></div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Notas (opcional)</label><textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} rows={2} placeholder="Notas..." className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm resize-none" /></div>
                {formErrors.submit && <p className="text-xs text-rose-500">{formErrors.submit}</p>}
                <div className="flex gap-3 pt-4"><button type="button" onClick={() => setAppointmentFormModal(null)} className="flex-1 py-3 rounded-xl border border-slate-200 font-bold text-slate-600">Cancelar</button><button type="submit" disabled={formSubmitting} className="flex-1 py-3 rounded-xl bg-primary text-white font-bold hover:bg-primary-dark disabled:opacity-50">{formSubmitting ? 'Guardando...' : 'Guardar'}</button></div>
              </form>
            </div>
          </>
        )}
          ...
          <>
            ...
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-2xl shadow-2xl p-5 sm:p-8 z-[61] max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto my-4">
              ...
              <form onSubmit={handlePatientFormSubmit} className="space-y-4">
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nombre completo</label><input type="text" value={patientFormData.name} onChange={(e) => setPatientFormData({ ...patientFormData, name: e.target.value })} placeholder="Nombre del paciente" className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" />{patientFormErrors.name && <p className="text-xs text-rose-500 mt-1">{patientFormErrors.name}</p>}</div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Email</label><input type="email" value={patientFormData.email} onChange={(e) => setPatientFormData({ ...patientFormData, email: e.target.value })} placeholder="email@ejemplo.com" className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" />{patientFormErrors.email && <p className="text-xs text-rose-500 mt-1">{patientFormErrors.email}</p>}</div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Teléfono</label><input type="tel" value={patientFormData.phone} onChange={(e) => setPatientFormData({ ...patientFormData, phone: e.target.value })} placeholder="+56 9 ..." className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" />{patientFormErrors.phone && <p className="text-xs text-rose-500 mt-1">{patientFormErrors.phone}</p>}</div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Fecha de nacimiento (opcional)</label><input type="date" value={patientFormData.birthDate} onChange={(e) => setPatientFormData({ ...patientFormData, birthDate: e.target.value })} className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm" /></div>
                <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Notas (opcional)</label><textarea value={patientFormData.notes} onChange={(e) => setPatientFormData({ ...patientFormData, notes: e.target.value })} rows={3} placeholder="Observaciones, alergias, etc." className="w-full rounded-xl border border-slate-200 py-2.5 px-4 text-sm resize-none" /></div>
                {patientFormErrors.submit && <p className="text-xs text-rose-500">{patientFormErrors.submit}</p>}
                <div className="flex gap-3 pt-4"><button type="button" onClick={() => setPatientFormModal(null)} className="flex-1 py-3 rounded-xl border border-slate-200 font-bold text-slate-600">Cancelar</button><button type="submit" disabled={patientFormSubmitting} className="flex-1 py-3 rounded-xl bg-primary text-white font-bold hover:bg-primary-dark disabled:opacity-50">{patientFormSubmitting ? 'Guardando...' : 'Guardar'}</button></div>
              </form>
            </div>
          </>
        )}
