...
const PatientList: React.FC<PatientListProps> = ({
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-primary hover:bg-primary-dark text-white rounded-xl font-bold text-sm shadow-md transition-colors"
          >
      ...
      {importResult && (
        <div
          className={`p-4 rounded-xl font-bold text-sm flex items-center gap-2 ${
            importResult.added > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'
          }`}
        >
          ...
          {importResult.added > 0
            ? `${importResult.added} paciente(s) añadido(s) desde el calendario`
            : 'No hay pacientes nuevos en las citas (todos ya están registrados)'}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="py-20 text-center bg-white rounded-3xl border border-slate-200 border-dashed">
          ...
          <button
            onClick={onNewPatient}
            className="mt-6 px-6 py-3 bg-primary text-white rounded-xl font-bold text-sm"
          >
            Añadir paciente
          </button>
        </div>
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => p.id && onDeletePatient(p.id)}
